...
function submit(){
	...
	function change_username_messi(){
		...
		{title: 'CHANGE PROFILE', titleClass: 'info', modal: true});  
...
$('#correctTextWrap').click(function (){
			...
			{title: 'All possible answers', titleClass: 'info', modal: true}); 
	...
	$('#correct_image').click(function(){
			...
			{title: 'Report problem', titleClass: 'info', modal: true}); 
...
function submit_artist_option(){					
	...
	{title: 'Add a new artist option', titleClass: 'info', modal: true}); 
}
...
function submit_songname_option(){
	...
	{title: 'Add a new song name option', titleClass: 'info', modal: true}); 					
}
	...
	if(!response.error){
		...
		for(var i in response.songnames){
			...
			$('#possibleSongnames').append( "<span id='song_votes" + response.songnames[i].id + "' style='width:50px'>" + response.songnames[i].votes + "</span> " + response.songnames[i].name + '<input type="button" id="song_upvote' + response.songnames[i].id +'" class ="btn btn-lg btn-success" value="Up" onclick="upvote_songname_answer('+ response.songnames[i].id +');" style="line-height: 15px; margin-left: 10px; margin-bottom: 5px;">' +
			 '<input type="button" id="song_downvote' + response.songnames[i].id +'" class ="btn btn-lg btn-danger" value="Down" onclick="downvote_songname_answer('+ response.songnames[i].id +');" style="line-height: 15px; margin-left: 10px; margin-bottom: 5px;"><br>'); 
		}				 
	}
	else{
		$('.allUnchecked').html('');
...
function send_new_artist_option(){
	...
	if(user_artist_option != "" && window.save_song_id > 0){
...
function send_new_songname_option(){
	...
	if(user_songname_option != "" && window.save_song_id > 0){
